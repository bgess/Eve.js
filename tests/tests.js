  ...
  test(".first should return the first of matching items", function() {
    ...
    Eve.scope('#click-me', function() {
      this.on('a', 'click', function(e) {
        this.first('a').html('clicked');
      });
    });
    
    simulate('click-me-a', 'click');
    var el = document.getElementById("click-me").getElementsByTagName("a")[0]
    ok(el.innerHTML === 'clicked', "Found the expected element.");
    
    Eve.scope('#els-list', function() {
      this.first('span')[0].className = 'clicked-first';
    });
